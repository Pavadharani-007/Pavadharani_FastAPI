...
# company
@app.post('/Employees/{Emp_id}/Company/{Comp_id}', tags=['Assignment Projects and Employees'])
def employee_to_company(company_id: int, employee_id: int, db: Session = Depends(get_db)):
    employee = db.query(models.Employee).filter(models.Employee.id == employee_id).first()
    ...
    db.commit()
    return 'Employee assigned to Company'
...
@app.get('/companies/{company_id}/employees/', response_model=schemas.Company, tags=['Company'])
